...
@app.get("/sort")
def sort_patient(
    sort_by: str = Query(..., description="Sort on the basis of field"),
    order: str = Query("asc", description="Sort in asc or desc order"),
):
    # these must match keys present in data
...
def create_patient(patient: Patient):
    ...
    # add new patient (id as key, rest as value)
...
def update_patient(patient_id: str, patient_update: PatientUpdate):
    ...
    # rebuild via Patient model to recompute bmi & verdict
...
def delete_patient(patient_id:str):
    ...
    JSONResponse(status_code=200,content={'message':'Patient deleted'})